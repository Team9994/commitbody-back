...
public class ExerciseCommentLikeServiceImpl implements ExerciseCommentLikeService {
    ...
    public String articleLike(Long articleId, Long memberId) {
        Optional<ExerciseCommentLike> commentLikeOptional = exerciseCommentLikeRepository.findByMemberIdAndArticleIdAndExerciseCommentIsNull(memberId, articleId);
        ...
        if (commentLikeOptional.isEmpty()){
            ...
            ExerciseCommentLike articleLike = ExerciseCommentLike.createArticleLike(member, article);
            getUpdateLikeCount(article,true);
            exerciseCommentLikeRepository.save(articleLike);
            return ADD;
        }else{          // 만약 좋아요한 데이터자 존재할때
            ExerciseCommentLike exerciseCommentLike = commentLikeOptional.get();
